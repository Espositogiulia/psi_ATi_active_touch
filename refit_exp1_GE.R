...
setwd("C:/data/gesposito/psiEvita/modelRefit/psi ATi")
...
data_list <- list(x=x,
                  ...
                  condition=condition)
#### Refit classic model ####
mod<-cmdstan_model("classical_model_exp1.stan",dir=paste(dir,sep=""))
fit_classical_exp1<-mod$sample(data=data_list,
                seed=123,
                chains=4,
                parallel_chains = 4,
                iter_warmup = 2500,
...
saveRDS(fit_classical_exp1,"fit_exp1_classical.rds")

#### Diagnose classic model####
fit_classical_exp1<-readRDS("fit_exp1_classical.rds")

stanfit <- rstan::read_stan_csv(fit_classical_exp1$output_files())
launch_shinystan(stanfit,rstudio = T)


fit_classical_exp1$diagnostic_summary()

 draws<-fit_classical_exp1$draws(
   variables=c(
     "mu_a","sigma_a","coef_a",
...
np<-nuts_params(fit_classical_exp1)
...
loo_classical_exp1<-fit_classical_exp1$loo(cores=5)
plot(loo_classical_exp1)
print(loo_classical_exp1)

rhat<-summary(draws)%>%select(rhat)
print(max(rhat))

##### Inference ######
 draws<-fit_classical_exp1$draws(format = "draws_df")
...
ggT <- ggplot(threshold, aes(x=value,color=Condition,fill=Condition)) +
  geom_density(alpha = 0.4,position = "identity",adjust = 1, show.legend=FALSE) +
...
ggsave(filename = "DensityNew_2102.tiff",plot=ggAll,device = "tiff",dpi=300,units="cm",width=16,height =12)
#ggsave(filename = "DensityNew_2102.png",plot=ggAll,device = "png",dpi=300,units="cm",width=15,height =10)
